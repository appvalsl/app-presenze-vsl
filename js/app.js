...
function normalizeLineName(value) {
  const u = String(value ?? '').trim().toUpperCase().replace(/\s+/g, ' ');
...
function showErrors(el, list) {
  ...
  el.innerHTML = `<ul style="margin:0; padding-left:18px;">${list.map((x) => `<li>${escapeHtml(x)}</li>`).join('')}</ul>`;
}
...
function renderLines() {
  if (!els.line) return;
  els.line.innerHTML = '<option value="">Seleziona...</option>' + LINES.map((line) => `<option value="${escapeHtml(line)}">${escapeHtml(line)}</option>`).join('');
}
...
function restoreAppState() {
  try {
    ...
    if (![1, 2, 3].includes(state.currentStep)) state.currentStep = 1;
...
function validateWizardStep(step) {
  ...
  if (step === 2) {
    ...
    if (sm == null || em == null) {
      if (setup.startTime || setup.endTime) errors.push('Orari non validi.');
    } else {
      const net = (em - sm) - setup.lunchMin - setup.snackMin - setup.stopsMin;
      if (net < 0) errors.push('Il tempo produttivo base è negativo.');
    }
...
function renderWizard() {
  ...
  if (els.wizardStepBadge) els.wizardStepBadge.textContent = `Step ${state.currentStep}/3`;
  if (els.wizardProgressBar) els.wizardProgressBar.style.width = `${(state.currentStep / 3) * 100}%`;
  ...
  if (state.currentStep === 3) updateSetupReview();
}
...
function updateSetupReview() {
  ...
  els.setupReview.innerHTML = `
    <div class="setup-review-row"><div class="setup-review-label">Linea</div><div class="setup-review-value">${escapeHtml(selectedLineText)}</div></div>
    <div class="setup-review-row"><div class="setup-review-label">Data</div><div class="setup-review-value">${escapeHtml(setup.date || '—')}</div></div>
    <div class="setup-review-row"><div class="setup-review-label">Inizio</div><div class="setup-review-value">${escapeHtml(setup.startTime || '—')}</div></div>
    <div class="setup-review-row"><div class="setup-review-label">Fine</div><div class="setup-review-value">${escapeHtml(setup.endTime || '—')}</div></div>
    <div class="setup-review-row"><div class="setup-review-label">Pausa pranzo</div><div class="setup-review-value">${escapeHtml(String(setup.lunchMin || 0))} min</div></div>
    <div class="setup-review-row"><div class="setup-review-label">Pausa snack</div><div class="setup-review-value">${escapeHtml(String(setup.snackMin || 0))} min</div></div>
    <div class="setup-review-row"><div class="setup-review-label">Fermi</div><div class="setup-review-value">${escapeHtml(String(setup.stopsMin || 0))} min</div></div>
    <div class="setup-review-row"><div class="setup-review-label">Nota fermi</div><div class="setup-review-value">${escapeHtml(note)}</div></div>
  `;
}
...
function finalMinutes(row) {
  const s = state.setup;
  if (!s) return 0;
  return Math.max(0, Number(row.workMin || 0) - Number(s.snackMin || 0) - Number(s.stopsMin || 0) - Number(row.eventoMin || 0) - Number(row.assembleaMin || 0) - Number(row.scioperoMin || 0));
}
...
function renderRows() {
  ...
  els.rowsBody.innerHTML = state.rows.map((r, idx) => {
    const stationOptions = ['<option value="">(Non assegnata)</option>', ...stations.map((station) => `<option value="${escapeHtml(station)}" ${station === r.postazione ? 'selected' : ''}>${escapeHtml(station)}</option>`)].join('');
    return `
      <tr data-idx="${idx}">
        <td><div class="row-name"><strong>${escapeHtml(r.cognome)} ${escapeHtml(r.nome)}</strong><small>${escapeHtml(r.idOperatore)} · ${escapeHtml(r.idCdc || '')}</small></div></td>
        <td>${Number(r.oreStandard || 0)}</td>
        <td><input data-field="workMinHours" type="number" step="0.25" min="0" value="${minutesToQuarterHours(r.workMin)}" /></td>
  ...
  if (els.rowsCount) els.rowsCount.textContent = `Righe: ${state.rows.length}`;
  if (els.setupSummary) {
    els.setupSummary.textContent = state.setup ? `Data: ${state.setup.date} · Linea: ${state.setup.line} · ${state.setup.startTime}-${state.setup.endTime} · Pranzo ${state.setup.lunchMin}m · Snack ${state.setup.snackMin}m · Fermi ${state.setup.stopsMin}m` : '';
  }
}
...
function updateSingleRowFinalCell(target) {
  ...
  if (finalCell) finalCell.textContent = minutesToHM(finalMinutes(state.rows[idx]));
}
...
function mapOperatorRecord(raw) {
  return {
    ...
    idOperatore: safeString(pickFirst(raw.idOperatore, raw.id_operatore, raw.codice_operatore, raw.codice)),
    idCdc: safeString(pickFirst(raw.idCdc, raw.id_cdc, raw.cdc, raw.centro_di_costo)),
    macroLineaProduzione: safeString(pickFirst(raw.macroLineaProduzione, raw.macro_linea_produzione)),
    lineaProduzione: normalizeLineName(pickFirst(raw.lineaProduzione, raw.linea_produzione, raw.line_name, raw.linea, raw.line)),
    postazione: safeString(pickFirst(raw.postazione, raw.station, raw.stazione)),
    oreStandard: Number(pickFirst(raw.oreStandard, raw.ore_standard, raw.standard_hours, 8)) || 8
  };
}

async function loadOperatorsFromDb() {
  const res = await state.supabase.from('operators').select('*');
  if (res.error) throw res.error;
  const rawData = res.data || [];
  state.allOperators = rawData.map(mapOperatorRecord).sort((a, b) => {
  ...
  console.log('Prime linee lette:', [...new Set(state.allOperators.map((o) => o.lineaProduzione).filter(Boolean))]);
}
...
function loadRowsForSelectedLine() {
  ...
  const operators = state.allOperators.filter((op) => normalizeLineName(op.lineaProduzione) === selectedLine);
  console.log('Linea selezionata:', selectedLine);
  console.log('Operatori filtrati:', operators.length);
  if (!operators.length) {
    const availableLines = [...new Set(state.allOperators.map((o) => o.lineaProduzione).filter(Boolean))];
    showErrors(els.setupErrors, ['Nessun operatore trovato per la linea selezionata.', `Linee presenti nella tabella operators: ${availableLines.join(', ') || 'nessuna'}`]);
    return;
  }
...
async function saveToDatabase() {
  ...
  const sessionPayload = {
    ...
    base_net_minutes: Math.max(0, workMinutesExcludingLunch(setup) - setup.snackMin - setup.stopsMin),
    created_by: state.currentUser.id
  };
  const sessionRes = await state.supabase.from('attendance_sessions').upsert(sessionPayload, { onConflict: 'work_date,line_name' }).select().single();
  if (sessionRes.error) throw sessionRes.error;
  const deleteRes = await state.supabase.from('attendance_rows').delete().eq('attendance_session_id', sessionRes.data.id);
  ...
  const rowsRes = await state.supabase.from('attendance_rows').insert(rowsPayload);
  if (rowsRes.error) throw rowsRes.error;
}

async function signIn() {
  console.log('click login intercettato');
...
async function handleSession(session) {
  ...
  if (!state.currentUser) {
    ...
    if (els.statusBadge) els.statusBadge.textContent = 'Accesso richiesto';
    if (els.subtitle) els.subtitle.textContent = 'Login';
    return;
  }
  els.authSection?.classList.add('hidden');
  els.btnLogout?.classList.remove('hidden');
  if (els.statusBadge) els.statusBadge.textContent = state.currentUser.email;
  if (els.subtitle) els.subtitle.textContent = 'Setup';
...
function bindQuickButtons() {
  ...
  [els.startTime, els.endTime, els.lunchMin, els.snackMin, els.stopsMin].filter(Boolean).forEach((input) => {
...
function bindWizardEvents() {
  ...
  [els.line, els.date, els.startTime, els.endTime, els.lunchMin, els.snackMin, els.stopsMin, els.stopsNote].filter(Boolean).forEach((element) => {
...
function bindMainEvents() {
  console.log('bindMainEvents', { btnLoginFound: !!els.btnLogin });
  ...
  els.btnSaveDb?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (els.statusBadge) els.statusBadge.textContent = 'Presenze salvate';
...
window.addEventListener('DOMContentLoaded', () => {
  init().catch((err) => {
    ...
    if (els.statusBadge) els.statusBadge.textContent = 'Errore';
